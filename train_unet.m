...
% Modern pipeline: unet (dlnetwork) + trainnet + custom loss (Weighted CE + Soft Dice)
% Assumes images 256x256 (RGB or grayscale) and binary masks (Polyp vs background).
...
%% Align by basename (do NOT abort; keep only valid pairs)
...
% Read mask and convert to categorical {background, Polyp}, robust to JPG artifacts.
M = imread(filename);
...
% strict=false: keep only matched pairs (recommended).
...
% Normalize basename: lowercase, strip common suffixes, drop non-alnum.
